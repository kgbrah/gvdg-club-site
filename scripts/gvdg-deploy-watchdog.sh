...
reassert() { # $1 = full origin/main sha, $2 = reason, $3 = dryrun(0/1)
  ...
  if ! ( cd "$build" && npm ci >>"$LOG" 2>&1 && npm run build >>"$LOG" 2>&1 && cp -R ./*.html ./*.js ./*.css home-app public-app tee-sign-preview-app members-app score-app img _headers CNAME site.webmanifest .pages-dist/ >>"$LOG" 2>&1 && mkdir -p .pages-dist/src/shared && cp src/shared/tee-sign-model.js .pages-dist/src/shared/ >>"$LOG" 2>&1 ); then
