...
test('admin member temporary PIN display is rendered by React from legacy create/reset state', () => {
  ...
  assert.match(showTempPin, /window\.__gvdgAdminMemberTempPinState = state/);
  assert.match(showTempPin, /gvdg:admin-member-temp-pin/);
  ...
  assert.match(membersList, /window\.__gvdgAdminMemberTempPinState/);
