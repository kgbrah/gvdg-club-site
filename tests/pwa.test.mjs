...
test('live scoring links back to the members dashboard', () => {
  ...
  assert.match(html, /<script type="module" src="score-app\/score-app\.js\?v=176"><\/script>/);
...
test('shared service worker caches app install assets and member fallback', () => {
  ...
  assert.ok(Number(cacheVersion[1]) >= 102, 'Rebuilt player-record bundles require v102 or newer');
