...
describe("countScores (exclusive mix)", () => {
  it("buckets holes by score-to-par without stacking an ace as an eagle", () => {
    // pars:  3  4  3  5     strokes: 1  2  2  7
    // ace, eagle, birdie, double+
    const b = countScores([3, 4, 3, 5], [1, 2, 2, 7]);
    expect(b).toEqual({ aces: 1, eagles: 1, birdies: 1, pars: 0, bogeys: 0, doubles_plus: 1 });
  });
