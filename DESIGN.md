...
- Spacing: `.score-glove-layout` is a column: `.score-glove-stage` holds weather, a 16:9 map (`max-height: 28dvh`), tools, and a flex hole-chip grid in the leftover band above the dock; `.score-glove-dock` is `34dvh` so scores stay in the one-handed thumb zone. `.score-glove-scores` scrolls inside the dock; `.hole-head` stays pinned to the bottom with nav buttons next to the hole label. The remaining-ft HUD sits bottom-left on the map, not in the dock.
