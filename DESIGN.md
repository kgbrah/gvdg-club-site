...
- States: migrated legacy fallback nodes must be absent, not hidden; auth mode is driven by `gvdg:member-auth-mode`; the bundled auth controller owns `gvdg:member-auth-ready`, `gvdg:member-profile-updated`, and login/session events; the bundled dashboard router owns `gvdg:select-dashboard-tab`, `gvdg:member-dashboard-opened`, and wrapper `dtab-off` state; the admin portal link renders only on the overview tab for admin members.
