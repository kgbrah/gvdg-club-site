...
- Variants: event card, casual round with share/add/manage/measure tools (event cards keep Share, Watch, and Measure), singles rows, doubles pair rows, matchplay status, dormie badge, hole map with a concrete tee pad aimed down the fairway and a compact chain-basket mark, PDGA C1/C2 rings around the basket, remaining-ft HUD, throw-measure lies, a hole-chip grid that paints birdie green / par yellow / bogey red as soon as a score is tapped (including the current hole), waiting-on-you hint, tee-sign highlight, score conflicts, score-target warning, and current-hole states.
- Spacing: `.score-glove-layout` is a column: `.score-glove-stage` holds weather, a 16:9 map that flexes into leftover height, tools, and a compact hole-chip grid; `.score-glove-dock` sizes to the scorecard (max `38dvh`) so a one-player guest card does not leave an empty thumb-zone gap. Browser Chrome uses `100svh`. `.score-glove-scores` scrolls when the card is long; `.hole-head` stays pinned to the bottom with nav buttons next to the hole label. The remaining-ft HUD sits bottom-left on the map, not in the dock.
- States: scorer changes, hole navigation, plus/minus steppers, and weather refreshes rerender through React; compass updates use the shared weather subscription helpers. Local GPS (`useDeviceFix`) drives remaining-to-basket without posting a location share. With a fix the HUD shows GPS remaining as the primary number and published hole length underneath; without a fix it shows hole length alone. Measure cycles drop A, mark B, then clear. Completed hole chips show strokes plus birdie/par/bogey and jump to that hole. One confirmation from anyone on the card locks it when holes agree; the recap is player totals, not an 18-hole table. Share builds a branded PNG of the card (course, holes, to-par) and prefers the phone share sheet with the image attached; with no scores yet it still shares the join or watch link. `src/score-app/score-view-model.js` derives scorecard choices, player/pair rows, current scores, relative labels, conflicts, totals, match status, finalize blockers, and UDisc export data so React-facing state is pure and testable.
- Accessibility: navigation and score steppers expose action labels, scorer selection has a label, tool buttons pair Lucide icons with text (Share and Measure use `aria-label` because labels are icon-only on the glove), and the remaining-ft HUD is a live status.
