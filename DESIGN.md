...
- Motion: the followed player's disc flies on the watch hole map the same way it does on the scorecard (tee or last lie → new mark, then into the basket on a score), using that player's saved plastic. Shot lines for every player on the selected hole stay on the map. Respect `prefers-reduced-motion`.
