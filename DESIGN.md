...
- Spacing: reuses `.card`, `.section`, `.btn`, `.weather-strip`, and `.lb` primitives with a compact `.watch-banner` and `.watch-actions` row. Follow chips sit under the hole map when more than one player has marked. Replay controls sit above the map on a finished round.
