...
- Motion: the followed player's disc flies on the watch hole map the same way it does on the scorecard (tee or last lie → new mark, then into the basket on a score), using that player's saved plastic. Live watch keeps shot lines for every player on the selected hole. Replay holds on the tee so the satellite can load, then reveals one throw at a time with a pause after each flight. Current, previous, and next hole satellite images are preloaded on watch and scorecard before they are shown; the rest of the layout warms in idle time. Respect `prefers-reduced-motion`.
