...
- Variants: login, forced-PIN, profile setup, overview, play, events, board, tee signs, and club tabs; auth/account tools dispatch credential, passkey, profile, and logout events while bundled member auth modules perform the secure flows.
...
- Motion: a small disc flies slowly along the hole map in SVG space — from the tee (or last lie) to a newly marked lie, then from that lie into the basket when a hole score is recorded. Respect `prefers-reduced-motion`. Other motion is press feedback and the weather wind-arrow.
