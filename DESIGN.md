...
| Disc/orange | `--disc-orange` | `#FF6B35` | `#FF6B35` | Live scoring disc plastic |
| Disc/blue | `--disc-blue` | `#004E89` | `#004E89` | Live scoring disc plastic |
| Disc/gold | `--disc-gold` | `#F7B801` | `#F7B801` | Live scoring disc plastic |
| Disc/forest | `--disc-forest` | `#2D5016` | `#2D5016` | Live scoring disc plastic |
| Disc/white | `--disc-white` | `#F4F4F9` | `#F4F4F9` | Live scoring disc plastic |
| Disc/ink | `--disc-ink` | `#1A1A2E` | `#1A1A2E` | Live scoring disc plastic |
| Disc/fire | `--disc-fire` | `#C8472E` | `#C8472E` | Live scoring disc plastic |
| Disc/pink | `--disc-pink` | `#E85D9A` | `#E85D9A` | Live scoring disc plastic |
| Disc/teal | `--disc-teal` | `#148F8F` | `#148F8F` | Live scoring disc plastic |
| Disc/purple | `--disc-purple` | `#6B4C9A` | `#6B4C9A` | Live scoring disc plastic |
| Disc/multicolor | `--disc-multicolor` | pie of disc plastics | pie of disc plastics | Six-slice factory rainbow disc |
| Disc/tiedye | `--disc-tiedye` | swirl of disc plastics | swirl of disc plastics | Dye-swirl disc from teal / pink / gold / purple / orange / blue |
...
- Structure: homepage header, logo, a compact public nav (Keep score, Events, Pro Shop, Members, a More menu for Ryder Cup / Blog / Help, and Donate), theme toggle, and mobile menu control render from the `home-app` React bundle into `homeReactPageChromeApp`; `nav.js` does not run on the homepage.
- Variants: desktop inline nav with More overflow, mobile collapsed nav (full list, including Home), mobile open nav, current-page link, keep-score link, donate link, light theme, dark theme, and scrolled header.
...
- Structure: Events, Ryder Cup, Pro Shop, and Blog mount a shared React-owned header, logo, compact public nav (Keep score, Events, Pro Shop, Members, More, Donate), theme toggle, and mobile menu into `publicReactPageChrome`; `nav.js` does not run on these public content pages.
- Variants: desktop inline nav with More overflow, mobile collapsed nav (full list, including Home), mobile open nav, current-page link, keep-score link, donate link, light theme, dark theme, and scrolled header.
...
- Variants: empty list, course row with optional location, and DiscGolfAPI import of every NC course plus courses within 250 miles of Greenville.
...
- Structure: React-owned header with logo link, compact public nav (Keep score, Events, Pro Shop, Members, More, Donate), mobile menu control, and theme toggle mounted into `membersReactPageChrome`. Signed-in phone and standalone PWA hide the entire public header so the bottom dock is the IA; the logo stays in-app (`gvdg-members.html`).
- Variants: desktop inline nav with More overflow, mobile collapsed nav, open mobile nav, signed-in phone (header hidden, dock only), light theme, and dark theme.
...
- Variants: login, forced-PIN, profile setup, overview, play, events, board, tee signs, club, and more tabs; the Events tab lists upcoming PDGA events within 100 miles (DiscGolfScene registration link, closest to the player's location first, Greenville until location is allowed) above club registration; an hourly worker refresh pulls newly posted events onto that list, and a Grok bot reports only the new ones; the More tab includes a live-scoring disc color picker (persisted at `/me/disc-color`) plus the dashboard theme builder; auth/account tools dispatch credential, passkey, profile, and logout events while bundled member auth modules perform the secure flows.
...
- Variants: empty course/layout states, selected setup options, back navigation, signed-in vs open-play home (Sign out vs Sign in), course search, an All-default search-radius dropdown (10 / 25 / 50 / 100 / 250 miles), mapped courses (tee + pin GPS so satellite can render) listed above unmapped ones regardless of distance, nearest-first within each group, a Mapped badge on the location line, a Google Maps directions control on each course row, and a catalog that includes every listed North Carolina course plus 250-mile neighbors.
...
- States: setup options use `aria-pressed` and tokenized borders/backgrounds for selected state; course pick asks the device for GPS and shows miles from that fix (search-radius dropdown too); until a fix arrives it lists every course with location only; mapped-first sort stays in effect during search and range filters; a Worker cron imports up to four unmapped UDisc layouts (pars + tee/pin GPS) every 15 minutes, matching similar names plus city/state and inheriting a mapped sibling's UDisc URL when the DiscGolfAPI title is just a rename (NC Wesleyan / Wesleyan College); once the statewide directory is exhausted it searches an 8-mile box around each leftover pin (North Carolina parks first) and reads the directory card's real name and city; with Ashe County Park jumping the nearest-first queue so it maps first; each mapped import busts the course catalog cache; leftover statewide misses are retried by that local search once, then left unmapped; legacy setup fallback nodes must be absent, not hidden.
...
- Structure: React-owned spectator screen with a live banner, hole map first (replay controls sit on the map for a finished round), then follow chips / hole grid / scores, pots, leaderboard, copy-watch-link, and round weather at the bottom. Keep-score stays on live rounds. The score controller fetches the public live snapshot and never joins the card. The hole map shows the followed player's marked lies and flies their disc as they mark or score. A finished round is a replay: Play/Pause and next/prev throw step through each lie, then into the basket, then the next hole.
- Variants: live event watch, casual-round watch by code, finished-round replay, archived casual round after the live object is gone, empty standings, and connection/final status. Hole maps overlay a landing-zone heatmap of saved marked lies (field density plus your own rings when signed in); Heat is on by default once three field samples exist and toggles without changing map size.
- Spacing: reuses `.card`, `.section`, `.btn`, `.weather-strip`, and `.lb` primitives with a compact `.watch-banner` and `.watch-actions` row. The map is the first block after the banner so watchers do not scroll to see the action; watch and replay maps are 50% taller than the previous 16/13.5 frame, capped to `min(70dvh, 42rem)` so they never overflow the viewport. Follow chips sit under the hole map. Replay controls sit above the map on a finished round. Round weather is last.
- States: `?watch=1` skips member login and `/join`; WebSocket snapshots rerender the table, lie marks, and disc flights; if the live object is gone, `GET /rounds/:code/live` serves durable D1 results as a final snapshot, including saved shot lines. Conflict alerts stay off for spectators. Auto-follow the most recently marked player; tapping a chip pins that player's lies. Last GPS stays on the map after the phone sleeps (dimmed pin); if GPS is gone, the last marked lie is the pin. GPS pins use the player's profile photo when one is set, otherwise initials. Finished rounds autoplay the followed player's throws hole-by-hole and can be paused, stepped, or jumped from the hole grid. Keep-score is hidden on a finished round.
- Accessibility: copy and keep-score are real controls; the live banner exposes connection text; the leaderboard table keeps rank/player/thru/result headers; follow chips expose `aria-pressed`; replay transport buttons expose play/pause and previous/next throw and hole labels.
- Motion: the followed player's disc flies on the watch hole map the same way it does on the scorecard (tee or last lie → new mark, then into the basket on a score), using that player's saved plastic. Watch and replay discs and lie marks are 50% larger than the scorecard marks. Live watch keeps shot lines for every player on the selected hole. Replay holds on the tee so the satellite can load, then reveals one throw at a time with a pause after each flight. Current, previous, and next hole satellite images are preloaded on watch and scorecard before they are shown; the rest of the layout warms in idle time. Respect `prefers-reduced-motion`.

### Scorecard View

- Structure: React-owned live hole screen. A landscape GPS hole map sits in the upper stage as a 16:9 card; a thumb-zone dock (about the bottom third) holds the score steppers, with hole `‹ ›` clustered around the hole label for one-handed reach. Optional tee-sign chip overlays the map. Round tools sit under the map, not on top of it. Leftover stage under the map is a live stats panel — exclusive scoring mix (ace / eagle / birdie / par / bogey / dbl+) plus FIR / C1R / C2R / parked / putting / scramble / birdie / par. On a one-row (solo) card the panel stays expanded, except on a phone-width screen where it starts collapsed to FIR, C1, C2, and birdie so the labels do not stack. On a multi-player card it starts as a compact strip and expands in place. The dock keeps one full stepper (you, or whoever you tap) and collapses the rest into a player rail, so the map stays large. Distance-to-basket, Mark lie, Undo, Mark tee, and Mark pin stay on the map. Club members can submit GPS for the tee pad and basket while they play; marks accumulate and a published consensus overlays the hole map. The score controller supplies derived rows and callbacks only.
- Variants: event card, casual round with share/add/manage/measure tools (event cards keep Share, Watch, and Measure), singles rows, doubles pair rows, matchplay status, dormie badge, hole map with a concrete tee pad aimed down the fairway and a compact chain-basket mark, PDGA C1/C2 rings around the basket, remaining-ft HUD, throw-measure lies, crowd-mapped tee/pin marks, a landing-zone heatmap of saved marked lies (field density gold→coral plus your rings) toggled from a Heat chip, a hole-chip grid that paints birdie green / par yellow / bogey red as soon as a score is tapped (including the current hole), waiting-on-you hint, tee-sign highlight, score conflicts, score-target warning, current-hole states, a solo-card live-stats fill, and a collapsed compact live-stats strip on multi-player cards.
- Spacing: `.score-glove-layout` is a column: `.score-glove-stage` holds weather, a 16:9 map, tools, and a compact hole-chip grid; `.score-glove-dock` sizes to the scorecard (max `38dvh`) so a one-player guest card does not leave an empty thumb-zone gap. Group cards use a `.score-player-rail` of compact chips plus one expanded stepper instead of stacking every row. The map keeps a full 16:9 frame (remaining-ft HUD, Mark lie, and Undo stay on it). On `.score-glove-layout.solo` the map stops growing (max `min(24dvh, 12rem)`) and `.live-round-stats` fills leftover height with a shrinking 3×3 stat grid (`grid-auto-rows: minmax(0, 1fr)`) so mix + FIR/C1/C2/parked/putts/scramble/birdie/par all stay on screen without inner scroll or clipping a row. On a group card `.live-round-stats.compact` stays `flex: 0 0 auto` under the map and expands in flow. Browser Chrome uses `100svh`. `.score-glove-scores` scrolls when the card is long; `.hole-head` stays pinned to the bottom with nav buttons next to the hole label. The remaining-ft HUD sits bottom-left on the map, not in the dock. Mark tee, Mark pin, and Mark lie stay in the space to the right of that HUD so a short map does not clip them. The glove also pads for whatever of the layout viewport an Android navigation bar covers, so the hole controls sit above the system buttons.
- States: scorer changes, hole navigation, plus/minus steppers, and weather refreshes rerender through React; compass updates use the shared weather subscription helpers. Local GPS (`useDeviceFix`) drives remaining-to-basket without posting a location share. Shared live GPS posts while scoring; last-known stays on the public map if the screen sleeps, and `watchPosition` restarts on visible/pageshow/online. GPS pins use the player's profile photo when one is set, otherwise initials. With a fix the HUD shows GPS remaining as the primary number and published hole length underneath; without a fix it shows hole length alone. Measure cycles drop A, mark B, then clear. Club members on a live card can Mark tee / Mark pin from their GPS; one sample per member per hole is stored, a 3-member cluster within 12 m publishes, and the live map overlays that consensus. Fuzzy GPS (>25 m) and off-hole marks are rejected. Marked lies persist into a public landing-zone heatmap for the layout (`GET /layouts/:id/heatmap?hole=N`); Heat stays an overlay, not a zoom mode, and is on by default at three field samples. Completed hole chips show strokes plus birdie/par/bogey and jump to that hole. Live stats recompute from the card's hole scores plus marked lies (sessionStorage throws, overlaid with the current hole) using the same exclusive play-stat mix as the members dashboard (`src/shared/play-stats.js`); an ace is only an ace, a birdie is only a birdie, a par is only a par. Mix counts and throw percentages update as soon as a score or lie changes. Group-card live stats toggle open/closed from the panel header. One confirmation from anyone on the card locks it when holes agree; the recap is player totals, not an 18-hole table. Share builds a branded PNG of the card (course, holes, to-par) and prefers the phone share sheet with the image attached; with no scores yet it still shares the join or watch link. `src/score-app/score-view-model.js` derives scorecard choices, player/pair rows, current scores, relative labels, conflicts, totals, match status, finalize blockers, format label, solo flag, and UDisc export data so React-facing state is pure and testable.
- Accessibility: navigation and score steppers expose action labels, scorer selection has a label, tool buttons pair Lucide icons with text (Share and Measure use `aria-label` because labels are icon-only on the glove), the remaining-ft HUD is a live status, the live-stats panel is a labeled live region, and the group-card collapse control exposes `aria-expanded`.
- Motion: a small disc flies slowly along the hole map in SVG space — from the tee (or last lie) to a newly marked lie, then from that lie into the basket when a hole score is recorded. The disc fill uses the member's saved plastic (`--disc-plate` / More → Live scoring disc), including solid colors plus Multicolor (pie slices) and Tie-dye (swirl). Respect `prefers-reduced-motion`. Other motion is press feedback and the weather wind-arrow.
...
- States: the score app and Events live detail render Round Weather through React using direct imports from `src/shared/weather-model.js`; public HTML shells must not load a standalone weather script or depend on a `GVDGWeather` global. Wind starts `North-up`, moves through `Listening...`, and changes to `Phone-relative` when device orientation produces a heading. Open-Meteo is the primary feed; when that quota is exhausted the live worker and the weather strip fall back to National Weather Service observations so the round still shows temp and wind.
