...
- Spacing: `.score-glove-layout` is a column: `.score-glove-stage` holds weather, a 16:9 map, tools, and a compact hole-chip grid; `.score-glove-dock` sizes to the scorecard (max `38dvh`, larger on `.players-3` / `.players-4`) so a one-player guest card does not leave an empty thumb-zone gap. The layout adds `players-1`…`players-4` from card size so the map shrinks instead of cropping — full 16:9 hole stays in view, with remaining-ft HUD, Mark lie, and Undo pinned on that frame. On `.score-glove-layout.solo` the map stops growing (max `min(24dvh, 12rem)`) and `.live-round-stats` fills leftover height with compact tiles that do not inner-scroll. On a group card `.live-round-stats.compact` stays `flex: 0 0 auto` under the map and expands in flow. Browser Chrome uses `100svh`. `.score-glove-scores` scrolls when the card is long; `.hole-head` stays pinned to the bottom with nav buttons next to the hole label. The remaining-ft HUD sits bottom-left on the map, not in the dock.
