...
- Motion: a small disc flies slowly along the hole map in SVG space — from the tee (or last lie) to a newly marked lie, then from that lie into the basket when a hole score is recorded. The disc fill uses the member's saved plastic (`--disc-plate` / More → Live scoring disc). Respect `prefers-reduced-motion`. Other motion is press feedback and the weather wind-arrow.
